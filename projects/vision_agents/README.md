...
Then, once a paper (a single one for simplicity) is selected, the system will produce a summary: both written and visual. For the latter, we use Gemini's NanoBanana model (find the implementation in [`utils`](./src/utils.py)) which has been known to produce great "whiteboard summaries" (check example below).
